...
public:
    ...
    // 创建报告详细记录
    ErrorCode createReportDetail(qint64 reportId, const QList<ReportDetail>& details);
    
    // 获取报告详细记录
    QList<ReportDetail> getReportDetails(qint64 reportId);
...
private:
    ...
    // 从查询结果构建报告详细对象
    ReportDetail buildReportDetailFromQuery(const QSqlQuery& query);
